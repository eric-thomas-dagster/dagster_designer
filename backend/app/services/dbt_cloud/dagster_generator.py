...
"""
    ...
    def _generate_git_clone_script(self, projects: List[Dict[str, Any]], project_repos: Dict[int, str]):
        ...
        content += "#\n"
        content += "# Windows: this is a bash script, not a batch/PowerShell file -- run it\n"
        content += "# from Git Bash or WSL (both include git and bash already). It won't run\n"
        content += "# by double-clicking, and PowerShell/cmd.exe can't execute it directly.\n\n"
        content += "set -e\n\n"
    ...
    def _generate_validation_script(self):
        ...
        content += "#\n"
        content += "# Windows: this is a bash script, not a batch/PowerShell file -- run it\n"
        content += "# from Git Bash or WSL (both include bash already). It won't run by\n"
        content += "# double-clicking, and PowerShell/cmd.exe can't execute it directly.\n\n"
        content += "set -e\n\n"
    ...
    def _generate_migration_summary(
        ...
        content += "   - **Windows:** these are bash scripts. Run them from Git Bash or WSL (cmd.exe/PowerShell can't execute them directly).\n"
...
4. Clone your dbt projects:
   - Run `./clone_dbt_projects.sh` to clone all repositories
     - **Windows:** run this from Git Bash or WSL, not cmd.exe/PowerShell -- it's a bash script.
...
- `.env` - Environment variables (gitignored)
- `clone_dbt_projects.sh` - Script to clone all dbt project repositories (bash; on Windows run via Git Bash or WSL)
- `validate_migration.sh` - Script to validate the migration (bash; on Windows run via Git Bash or WSL)
