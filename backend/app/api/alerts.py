...
async def list_alerts(project_id: str):
    """Return all alert policies for the project. For local projects,
    reads the YAML file. For Dagster+ projects, queries the deployment's
    live alert configuration via GraphQL (`alertPoliciesOrError`).
    Falls back to empty + a hint if the query fails so the panel stays
    usable across schema versions."""
    ...
    if getattr(project, "is_dagster_plus", False):
        try:
            ...
            return AlertsFile(
                path=f"(couldn't fetch from Dagster+: {e})",
                policies=[],
            )
        policies = _policies_from_graphql(data)
        return AlertsFile(path="(live from Dagster+)", policies=policies)
